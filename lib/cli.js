...
var apiSettings = {
    API_URL: PROJECT_PROTOCOL + '://' + PROJECT_NAME + '.' + PROJECT_DOMAIN + settings.prefix,
    API_VERSION: 'v1',
...
var refreshTokens = function(){
    ...
    .then(function(body){
        ...
        if(Math.abs(now.getTime() - rem.getTime()) > 30 * 1000) {
            console.error('Warning: the local and remote clocks seems to be out of sync.'.yellow);
            ...
            console.error(('Remote time ' + rem.toISOString()).yellow);
        }
...
//XQLint Command
var lintCmd = cli.command('xqlint <path>');
lintCmd.option(
...
.action(function(){
    ...
    .catch(function(error){
        console.log('Failed to authenticate. Please run: 28 login <your email>'.red);
...
.action(function(projectName){
    ...
    .then(function(){
        var projectToken = settings.project_tokens['project_'+projectName];
        if(projectToken) {
            ffs.mkdirRecursiveSync('modules', 511);
            ffs.mkdirRecursiveSync('public', 511);
            ffs.mkdirRecursiveSync('private', 511);
            var project = API.Project(apiSettings.getProjectAPI(projectName), projectToken);
            Q.all([
                project.listFiles(),
                ffs.readdirRecursive('modules', true, 'modules'),
