...
*/
    ...
    var logDiff = function(diff, simulate){
        var msg = '<' + loc(diff.isOriginRemote) + '>/' + diff.query;
        msg += ' -> <' + loc(!diff.isOriginRemote) + '>/' + diff.query;
        if(simulate === true) {
            msg += ' (simulate)';
        }
        if(diff.type === 'write') {
        ...
        diffs.forEach(function(diff){
            if(simulate) {
                ...
                if(diff.isOriginRemote) {
                    promises.push(writeLocalQuery(projectName, projectToken, localProjectPath, diff.query));
                } else {
                    promises.push(writeRemoteQuery(projectName, projectToken, localProjectPath, diff.query));
                }
            } else {
                if(diff.isOriginRemote) {
                    promises.push(deleteLocalQuery(diff.query));
                } else {
                    promises.push(deleteRemoteQuery(projectName, projectToken, diff.query));
                }
            }
        });
    ...
    var sync = function(localQueries, remoteQueries, projectName, projectToken, localProjectPath, overwrite, deleteOrphaned, simulate, isOriginRemote){
        var that = this;
        var diffs = [];
    ...
    this.download = function(projectName, projectToken, localProjectPath, overwrite, deleteOrphaned, simulate, ignore){
        ...
        .then(function(queries){
            ...
            sync(localQueries, remoteQueries, projectName, projectToken, localProjectPath, overwrite, deleteOrphaned, simulate, true);
        });
    };
};