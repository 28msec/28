...
var $28 = (function() {
    ...
    $28.prototype.upload = function(
        projectName, projectToken, localProjectPath,
        overwrite, deleteOrphaned, simulate, ignore, pathToJSONiqModule) {
        var that = this;
        if(pathToJSONiqModules) {
            var manifestPath = pathToJSONiqModule + '/28.json';
            var root = path.dirname(path.resolve(manifestPath)) + '/';
            ...
            _.forEach(manifest.dependencies, function(hash, path){
                ...
                return that.sync('upload', projectName, projectToken, base, overwrite, deleteOrphaned, simulate, ignore)
                    .then(function(){
    ...
    $28.prototype.install = function() {
        //TODO: install & deploy modules recursively
        var defered = Q.defer();
    ...
    $28.prototype.getQueryTrace = function(projectName, token, path) {
        return this.api.Queries(projectName).getQueryTrace({ token: token, queryPath: path });
    };
    ...
    $28.prototype.updateDatasource = function(projectName, name, category, newname, token, difault, credentials){
        return this.api.Datasources(projectName).updateDatasource({
            category: category,
            datasource: name,
            name: newname,
            token: token,
