...
exports.Client = function($28, projectName, projectToken, localProjectPath){
    ...
    this.sync = function(overwrite, deleteOrphaned, simulate, ignore, isOriginRemote){
        ...
        Q.all([
            vfs.getLocalQueries(localProjectPath),
            vfs.getRemoteQueries($28, projectName, projectToken)
        ])
    ...
    this.truncate = function(simulate, ignore){
        var defered = Q.defer();
        localProjectPath = normalizeLocalProjectPath(localProjectPath);
        vfs.getRemoteQueries(projectName, projectToken).then(function(queries){
